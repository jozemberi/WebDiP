<?php
	...
	if(isset($_GET['id_konf']))$id_konfiguracije=$_GET['id_konf'];
	
	if(isset($_POST['akcija'])){
		$narucitelj=$_SESSION['username'];
		$id_konfiguracije=$_POST['id_konf'];
		$sqlInsert = mysql_query ("INSERT INTO narudzba VALUES (default, '$narucitelj', $id_konfiguracije, 1, 1, now())") or die(mysql_error());	
	
	}
	
	else{
		$sql = "SELECT konfiguracija.id_konfiguracije, konfiguracija.cijena AS ukupna_cijena, korisnik.username AS kreator, konfiguracija.per_naziv AS naziv, osnovne_konfiguracije.model AS model, boje.naziv AS boja,
				...
				FROM korisnik INNER JOIN konfiguracija INNER JOIN osnovne_konfiguracije INNER JOIN mjenjac INNER JOIN boje
				INNER JOIN marka_automobila INNER JOIN motor INNER JOIN tip_motora ON korisnik.username=konfiguracija.kreator_konfiguracije AND osnovne_konfiguracije.mjenjac=mjenjac.id_mjenjac AND konfiguracija.boja = boje.id_boje AND
	...
	if(!$konfiguracija){
		...
		$smarty->assign("naslov", "Pregled vlastite konfiguracije");
		...
		$smarty->display("jozemberi_pregled_vlastite_konfiguracije.tpl");
