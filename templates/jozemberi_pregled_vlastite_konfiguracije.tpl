...
{literal}
	 ...
	 <script type="text/javascript">
				...
				$.ajax({
					...
					success: function(xml){
						...
						$(xml).find('komentar').each(function(){
							...
							$(this).attr('dat_i_vrij_komentara') + '</div> <div class="avatarKor"><img src="thumbs/' + 
							$(this).attr('avatar') + 
		...
		$('#naruciKonfiguracijuForm').submit(function(e){	
			e.preventDefault();
			var ok = confirm("Želite li naručiti ovu konfiguraciju?");
			if(ok==true){
				var podaci='akcija=naruci&id_konf=' + id_konf;
				$.ajax({
						type: "POST",
						url: "jozemberi_pregled_vlastite_konfiguracije.php",
						data: podaci,
						//dataType: 'xml',
						success: function(){
							alert('Konfiguracija je naručena! Obavjest o promijeni statusa naručene konfiguracije dobit ćete na email.');
	...
	<br />
				...
				<table align='center' class='{#table_class#}'>
					<tr>
						...
						<td> <b> Ukupno:</b> {$konfiguracija.ukupna_cijena} kn </td>
						<td align='right'><form id="naruciKonfiguracijuForm" method="POST" action="#">
						<input type="submit" id="naruciKonfiguraciju" value="Naruči ovu konfiguraciju" /> </form>
						</td>
					</tr>
					...
					<tr> <td><b>Cijena:</b> {$konfiguracija.cijena} kn </td> <td><b>Boja:</b> {$konfiguracija.boja} </td></tr>
					{* Gume *}
					...
					{*	<td rowspan='3'> <img align='right' height='120px' width='120px' src='{$felge[i].slika}' alt='Felge'/></td> *}
